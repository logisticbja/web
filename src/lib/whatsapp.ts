...
// Dipakai tombol "Tanya CS" di halaman Jadwal Kapal — khusus buat jadwal yang ETA-nya
// kosong (1 kapal transit ke beberapa Wilayah, ETA per tujuan beda-beda). Nyebut rute &
// nama kapal langsung di pesannya biar CS gak perlu nanya ulang konteksnya.
export function buildScheduleMessage(route: string, ship?: string) {
  const shipPart = ship ? ` (kapal ${ship})` : "";
  const msg = `Halo BJA Logistic, saya ingin tanya estimasi waktu tiba (ETA) untuk jadwal kapal rute ${route}${shipPart}. Bisa bantu info lebih detail?`;
  return WA_BASE + encode(msg);
}
